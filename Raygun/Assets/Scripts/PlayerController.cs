...
public class PlayerController : MonoBehaviour
{
    ...
    bool m_Ducking;
    float m_cloudCd = 0.04f;
    float m_cloudCdTimer;
    int m_timesJumped;
    //hp bar
    #endregion
    ...
    void Update()
    {
        ...
        m_cloudCdTimer = Functions.Cooldown(m_cloudCdTimer);
    }
    ...
    void DetectAnimationState()
    {
        ...
        else
        {
            ...
            {
                animator.SetBool("IsMoving", true);
                if (m_cloudCdTimer == 0 && m_Grounded)
                {
                    SpawnCloud();
                    m_cloudCdTimer += m_cloudCd;
                }
            }
            else
            {
    ...
    void SpawnCloud()
    {
        Instantiate<GameObject>(cloud, new Vector2(transform.position.x, transform.position.y - 0.47f), Quaternion.identity);
    }
    void SpawnCloudJump()
    {
        Instantiate<GameObject>(cloud, new Vector2(transform.position.x - 0.1f, transform.position.y - 0.47f), Quaternion.identity);
        Instantiate<GameObject>(cloud, new Vector2(transform.position.x, transform.position.y - 0.47f), Quaternion.identity);
        Instantiate<GameObject>(cloud, new Vector2(transform.position.x + 0.1f, transform.position.y - 0.47f), Quaternion.identity);
    }
    #endregion
